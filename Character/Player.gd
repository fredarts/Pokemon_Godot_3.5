...
const LandingDustEffect = preload("res://Character/LandingDustEffect.tscn")
...
onready var door_ray = $DoorRayCast2D3


var jumping_over_ledge: bool = false

var initical_position : Vector2 = Vector2(0,0)
...
func move(delta):
	...
	if door_ray.is_colliding():
		...
		if percent_moved_to_next_tile >= 1.0:
			position = initical_position + (input_direction * TILE_SIZE)
			...
			position = initical_position + (TILE_SIZE * input_direction * percent_moved_to_next_tile)
		...
		if percent_moved_to_next_tile >= 2.0:
			position = initical_position + (input_direction * TILE_SIZE * 2)
			percent_moved_to_next_tile = 0.0
			is_moving = false
			...
			position.y = initical_position.y + (-0.96 - 0.53 * input + 0.05 * pow(input, 2))
		...
		percent_moved_to_next_tile += walk_speed * delta
		if percent_moved_to_next_tile >= 1.0:
			position = initical_position + (TILE_SIZE * input_direction)
			percent_moved_to_next_tile = 0.0
			is_moving = false
